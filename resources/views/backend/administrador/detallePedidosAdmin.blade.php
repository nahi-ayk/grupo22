...
<div class="container my-5">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h1 class="h3 fw-bold admin-titulo">Detalle del Pedido #{{ $pedido->numero_pedido }}</h1>
            <p class="text-muted admin-subtitulo fs-2">
            Cliente: {{ $pedido->usuario ? $pedido->usuario->nombre . ' ' . $pedido->usuario->apellido : 'Usuario Desconocido' }}
            </p>
        </div>
        <a href="{{ url()->previous() }}" class="btn btn-custom-colores">Volver</a>
    </div>
                ...
                <div class="row g-4">
                    ...
                    <div class="col-md-6">
                        ...
                        @if(strtolower($pedido->estado) == 'carrito')
                            <p class="mb-0 text-secondary fw-medium"><i class="bi bi-wallet2"></i> A definir</p>
                        @else
                            <p class="mb-0 fw-medium">{{ $pedido->metodoPago->descripcion ?? $pedido->metodoPago->nombre ?? 'No especificado' }}</p>
                        @endif
                    </div>
